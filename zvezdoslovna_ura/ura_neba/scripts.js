let nanovoTimer;
function nanovo() {
    clearTimeout(nanovoTimer); // počisti star časovnik
    nanovoTimer = setTimeout(() => {
        Celestial.display(privzetiConfig);
    }, 500);
}
...
var config = { 
  ...
  stars: {
    ...
    designationType: "desig",  // Which kind of name 
...
const privzetiConfig = JSON.parse(JSON.stringify(config));

let dataTimer;
// posluša prejete podatke
window.addEventListener("message", (event) => {
    const podatki = event.data.podatki;
    console.log("Prejeti podatki:", podatki);

    clearTimeout(dataTimer); // počisti star časovnik
    dataTimer = setTimeout(() => { // pokliči funkcijo po ~250 ms
        /*Celestial.date(new Date(podatki.IČas))
        Celestial.location([podatki.IPolŠ, podatki.IPolD])*/
        Celestial.skyview({date:new Date(podatki.IČas), location:[podatki.IPolŠ, podatki.IPolD]})
        Celestial.reproject({projection:podatki.proj})

        /*// izbriši star SVG in nariši znova
